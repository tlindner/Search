...
-(void)main {
	@try {
		...
		logString( @"Search Live: Starting work\n" );
		NSFileManager *fm = [NSFileManager defaultManager];
		NSString *df = [fm applicationSupportDirectory];
		NSString *databasePath = [df stringByAppendingPathComponent:@"live.sqlite"];
		...
		if (sql_err == 0) {
            ...
            if (stmt != NULL) {
                ...
                while (true) {
                    ...
                    if (count == 50) {
                        if ([self isCancelled]) {
                            ...
                            [[NSApp delegate] performSelectorOnMainThread:@selector(sendLiveArray:) withObject:array waitUntilDone:YES];
                            [array removeAllObjects];
                            count = 0;
                        }
                    }
                ...
                logString( @"Search Live: prepare failed\n" );
            }
        }
		else {
			logString( @"Search Live: failed to open database\n" );
		}
        
		sqlite3_close( ppDb );
		logString( @"Search Live: Ending work\n" );
