...
-(void)main {
	@try {
		...
		sqlite3 *ppDb;
		sqlite3_stmt *stmt;
		logString( @"Search AIT: Starting work\n" );
		...
		if (sql_err == 0) {
            ...
            if (stmt != NULL) {
                ...
                while (true) {
                    ...
                    if (count == 50) {
                        if ([self isCancelled]) {
                            ...
                            [[NSApp delegate] performSelectorOnMainThread:@selector(sendAITArray:) withObject:array waitUntilDone:YES];
                            [array removeAllObjects];
                            count = 0;
                        }
                    }
                ...
                logString( @"Search AIT: prepare failed" );
            }
        }
		else {
			logString( @"Search AIT: failed to open database" );
		}

        sqlite3_close( ppDb );
        logString( @"Search AIT: Ending work\n" );
