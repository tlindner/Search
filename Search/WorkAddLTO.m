...
-(void)main {
	@try {
		...
		logString( @"Add LTO: Starting work\n" );
		...
		if (sql_err == 0) {
			...
			if (fp != nil) {
				...
				if (stmt != NULL) {
					...
					size_t len;
					char *line_size, *line_segment, *line_tape, *line_filename, *line_path;
					int lines = 0;
					
					while( (line_path = fgetln( fp, &len)) != NULL )
					{
						line_path[len-1] = 0;
						line_size = line_path + len - 2;
						
						while( line_size > line_path && line_size[0] != ' ' ) line_size--;
						
						line_segment = line_size - 1;
						line_size[0] = 0;
						...
						if ((lines++ % 50000) == 0) {
							logwindowf( "Add LTO: Processed %d lines\n", lines );
						}

						if (strncmp(line_path, "/Volumes", 8) == 0) {
							line_path += 8;
						}
					...
					logString( @"Add LTO: Prepared failed" );
				}
			}
			else {
				logString( @"Add LTO: failed to open LTO file." );
			}

			fclose(fp);
		}
		else {
			logString( @"Add LTO: failed to open database" );
		}

		sqlite3_close( ppDb );
		logString( @"Add LTO: Ending work\n" );
